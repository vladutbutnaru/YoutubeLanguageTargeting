...
public class ChannelSectionLocalizations {
    ...
    private static List<ChannelSection> channelSectionList;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            switch (action) {
                case SET:
                    setChannelSectionTargeting(getId("channel section")
                          );
                    break;
                case DELETE:
                    deleteChannelSectionTargeting(getId("channel section"));
                    break;
                case LIST:
...
private static void setChannelSectionTargeting(String id) throws IOException{
   ChannelSectionListResponse channelSectionListResponse = youtube.channelSections().
            list("snippet,contentDetails,targeting").setId(id).execute();

if (channelSectionListResponse.getItems().isEmpty()) {
            System.out.println("Can't find a channel section with ID: " + id);
            return;
        }
 ChannelSection channelSection = channelSectionListResponse.getItems().get(0);
...
languages.add("es");
 ...
 channelSection.getTargeting().setCountries(countries);
 channelSection.getTargeting().setLanguages(languages);
ChannelSection channelSectionResponse = youtube.channelSections()
            .update("snippet,contentDetails,targeting", channelSection).execute();
// Print information from the API response.
        System.out.println("\n================== Updated Channel Section ==================\n");
        System.out.println("  - ID: " + channelSectionResponse.getId());
    
         countries = channelSectionResponse.getTargeting().getCountries();
         ...
         System.out.println("No Targeting Countries Found");
         }
         ...
         System.out.println("No Targeting Languages Found");
         }
        System.out.println("\n-------------------------------------------------------------\n");
 }
...
private static void deleteChannelSectionTargeting(String id){}
    /**
    ...
    private static void listChannelSections() throws IOException{
    
     ChannelSectionListResponse channelSectionListResponse = youtube.channelSections().
            list("snippet,contentDetails,targeting").setMine(Boolean.TRUE).execute();

       System.out.println("\n================== CHANNEL SECTIONS ==================\n");
     channelSectionList = channelSectionListResponse.getItems();
        for(ChannelSection section : channelSectionList){
            if(section.getSnippet().getType().equals("singlePlaylist")){
        System.out.println(section.getId() + " -> " + section.getContentDetails().getPlaylists().get(0) + " -> " + section.getSnippet().getType());
           ChannelSectionTargeting targeting =  section.getTargeting();
        try{
       ...
       List<String> languages = targeting.getLanguages();
      
       System.out.println("Languages: ");
       for(String country : languages){
       System.out.println("------ " + country);
       }
        }
        catch(Exception e){
    ...
    private static void listChannelSectionLocalizations(String channelSectionId) throws IOException {
        // Call the YouTube Data API's channelSections.list method to retrieve channel sections.
        ChannelSectionListResponse channelSectionListResponse = youtube.channelSections().
            list("snippet,contentDetails,targeting").setId(channelSectionId).execute();
       ...
       ChannelSectionTargeting targeting =  channelSection.getTargeting();
       List<String> countries = targeting.getCountries();
       System.out.println("\n================== Countries ==================\n");
       for(String country : countries){
       System.out.println(country);
    ...
    private static String getId(String resource) throws IOException {
        ...
        System.out.println("You chose " + id + " for targeting.");
        return id;
    }
    ...
    private static String getActionFromUser() throws IOException {
        ...
        System.out.print("Options are: 'set'");
        BufferedReader bReader = new BufferedReader(new InputStreamReader(System.in));
